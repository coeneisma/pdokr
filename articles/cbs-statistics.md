...
dataset with hundreds of figures per neighborhood — population, income,
...
population density across the neighborhoods of Amsterdam.
...
layer, then read the `buurten` (neighborhoods) inside it.
...
A quantile classification keeps each color class roughly equal in size,
which works well for a skewed variable like density. Neighborhoods
without a value (water, parks, industry) are drawn in gray.
...
The same map interactively — hover for the neighborhood name and click
