...
#' @param datetime Optional temporal filter: a single year (e.g. `2026`), an OGC
#'   datetime string, or an interval such as `"2020-01-01/2025-12-31"`.
...
#' @examples
#' \donttest{
#' # Municipalities for 2024, in RD New
#' pdok_read(
...
#' }
#' @export
pdok_read <- function(dataset, layer, bbox = NULL, datetime = NULL,
  ...
  out <- if (!is.null(resolved$ogc)) {
    read_ogc(resolved$ogc, layer, bbox, datetime, max_features)
  ...
  if (nrow(out) == 0L) {
    cli::cli_warn(c(
      ...
      "i" = "Check the layer id, and any {.arg bbox} or {.arg datetime} filter."
    ))
    return(out)
  }
